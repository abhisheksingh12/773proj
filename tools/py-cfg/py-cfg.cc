...
const char usage[] =
"py-cfg version of 18th October, 2009\n"
"\n"
"py-cfg [-d debug]\n"
...
int main(int argc, char** argv) {
  ...
  Str parses_filename = "", grammar_filename = "", trace_filename = "";
  Strs evalcmdstrs;
  Postreamps evalcmds;
  U eval_every = 1;
  ...
  int chr;
  while ((chr = getopt(argc, argv, "A:CDEF:G:H:I:N:R:T:X:Z:a:b:d:e:f:g:h:m:n:r:t:w:x:z:")) 
	 != -1)
    switch (chr) {
    case 'A':
  ...
  { 
    ...
    if (!is)
      std::cerr << "# Error in " << argv[0] 
		<< ", can't open grammar file " << argv[optind] << std::endl;
    assert(is);
    is >> g;
  }
  ...
  if (debug >= 1000)
    std::cerr << "# py-cfg Initial grammar = " << g << std::endl;

  pycky parser(g);

  gibbs_estimate(g, trains, evalcmds, eval_every,
